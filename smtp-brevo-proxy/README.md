...
该服务在本地提供一个受限 SMTP 入口，接收 New API 生成的邮箱验证邮件，从 MIME 正文中解析 6 位十六进制验证码，渲染 Partokens 英文邮件模板，再通过 Brevo Transactional Email API 发送。
...
- 无法解析验证码、非法发件域名或收件人数超限：返回永久错误 `550`。
