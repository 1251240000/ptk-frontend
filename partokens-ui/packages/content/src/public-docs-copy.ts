...
export const publicDocsCopy: Record<AppLocale, DocsPageCopy> = {
  'zh-CN': {
    ...
    browseTitle: '按任务浏览', browseBody: '选择当前要完成的事情；后续内容将沿用这套目录逐页补齐。', referenceTitle: '先完成一次兼容调用', referenceBody: 'Partokens 使用统一的 OpenAI 兼容入口。选择一种调用方式，替换模型名后即可验证接入。',
  ...
  'zh-TW': {
    ...
    browseTitle: '依任務瀏覽', browseBody: '選擇目前要完成的事情；後續內容會沿用這套目錄逐頁補齊。', referenceTitle: '先完成一次相容呼叫', referenceBody: 'Partokens 使用統一的 OpenAI 相容入口。選擇一種呼叫方式，替換模型名稱後即可驗證接入。',
  ...
  en: {
    ...
    browseTitle: 'Browse by task', browseBody: 'Choose what you need to do now. Future content will follow this structure as each page is completed.', referenceTitle: 'Complete one compatible call', referenceBody: 'Partokens provides one OpenAI-compatible entry point. Choose a client, replace the model name, and verify the integration.',
  ...
  ja: {
    ...
    browseTitle: 'タスクから探す', browseBody: '今行いたい作業を選びます。今後のページもこの構成に沿って追加します。', referenceTitle: '互換リクエストを一度完了する', referenceBody: 'Partokens は統一された OpenAI 互換入口を提供します。呼び出し方法を選び、モデル名を置き換えて接続を確認します。',
  ...
  ru: {
    ...
    browseTitle: 'Поиск по задаче', browseBody: 'Выберите текущую задачу. Следующие страницы будут добавляться в эту структуру.', referenceTitle: 'Выполните один совместимый вызов', referenceBody: 'Partokens предоставляет единую OpenAI-совместимую точку входа. Выберите способ вызова, замените модель и проверьте подключение.',
  ...
  fr: {
    ...
    browseTitle: 'Parcourir par tâche', browseBody: 'Choisissez votre tâche actuelle. Les prochaines pages suivront cette structure.', referenceTitle: 'Effectuer un appel compatible', referenceBody: 'Partokens fournit un point d’entrée compatible OpenAI. Choisissez une méthode, remplacez le modèle et vérifiez l’intégration.',
  ...
  vi: {
    ...
    browseTitle: 'Duyệt theo công việc', browseBody: 'Chọn việc bạn cần làm lúc này. Nội dung tiếp theo sẽ được bổ sung theo cấu trúc này.', referenceTitle: 'Hoàn tất một yêu cầu tương thích', referenceBody: 'Partokens cung cấp một điểm vào tương thích OpenAI. Chọn cách gọi, thay tên mô hình và xác minh kết nối.',
