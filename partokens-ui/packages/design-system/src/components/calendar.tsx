...
import { enUS, fr, ja, ru, vi, zhCN, zhTW } from 'date-fns/locale'
...
function calendarDayValue(date: Date): string {
...
function Calendar({
  ...
  return (
    <DayPicker
      ...
      className={cn(
        'group/calendar bg-background p-3 [--cell-size:--spacing(8)] in-data-[slot=card-content]:bg-transparent in-data-[slot=popover-content]:bg-transparent [&_th]:!h-auto [&_th]:!border-0 [&_th]:!p-0 [&_th]:!text-center [&_th]:!text-muted-foreground [&_td]:!h-auto [&_td]:!border-0 [&_td]:!p-0 [&_tbody_tr:hover]:!bg-transparent',
      ...
      classNames={{
        ...
        weekday: cn('flex-1 rounded-md text-[0.8rem] leading-[1.2rem] font-normal text-muted-foreground select-none', defaultClassNames.weekday),
      ...
      components={{
        Root: ({ className, rootRef, ...rootProps }) => <div data-slot='calendar' ref={rootRef} className={cn(className)} {...rootProps} />,
...
function CalendarDayButton({
  ...
  return (
    <Button
      ...
      variant='ghost'
      size='icon'
      data-day={calendarDayValue(day.date)}
