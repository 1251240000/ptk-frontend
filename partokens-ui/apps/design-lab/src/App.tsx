...
const zh = {
  ...
  heroEyebrow: '面向每一种 AI 工作', heroTitle: '你的 AI 工作区，从对话到创作。', heroBody: '在一个账户里对话、生图、管理 API 密钥，并清楚掌握每一次使用。', openConsole: '立即使用', viewDocs: '查看接入文档',
  previewWorkspace: '对话工作区', previewNewChat: '新对话', previewQuestion: '帮我整理一份产品发布检查清单。', previewAnswer: '可以。我会按发布前验证、上线步骤和回滚准备三个部分整理。', previewPrompt: '输入消息，开始对话', localConversation: '对话记录仅保存在本地',
  endpoint: '接入点', modelRoute: '模型选择', signInForModels: '登录后选择可用模型',
  quickEntry: 'GET STARTED', workspaceChoice: '从你现在要做的事开始', getStartedBody: '无需先配置复杂流程，选择工作方式即可进入对应页面。', startChat: '发起一次对话', startChatBody: '使用本地会话历史和流式响应。', createImage: '进入生图工作台', createImageBody: '在本地画布中生成、连接和整理图片。', createKey: '创建 API 密钥', createKeyBody: '按模型、额度和有效期限制访问。',
  ...
  controlFeatureKicker: '用量与接入', controlFeatureTitle: '密钥、余额和日志始终有据可查', controlFeatureBody: '创建受限密钥，查看余额与用量，并从使用日志追踪每次请求。', controlFeatureNote: '账户数据', accountOverview: '账户概览', availableBalance: '可用余额', monthUsage: '本月用量', restrictedKey: '受限密钥', requestLog: '请求日志', accountRequired: '登录后读取',
  valuesEyebrow: 'WHY PARTOKENS', valuesTitle: '少一点切换，多一点确定', compatible: '兼容', compatibleBody: '使用熟悉的 OpenAI 兼容接口接入。', unified: '统一', unifiedBody: '对话、生图、密钥和用量集中管理。', transparent: '透明', transparentBody: '余额、用量与请求日志清楚可见。', localFirst: '本地', localFirstBody: '聊天记录默认留在你的当前设备。', controlled: '可控', controlledBody: '按模型、额度与有效期约束密钥。', multilingual: '多语言', multilingualBody: '产品界面与使用文档同步国际化。',
  faqEyebrow: 'FAQ', faqTitle: '开始之前，你可能想知道', faqBody: '关于隐私、账户安全和 API 接入的常见问题。', faq1q: 'Partokens 可以直接开始聊天和生图吗？', faq1a: '可以。登录后从控制台进入对话或生图工作台即可开始。', faq3q: '聊天记录会上传到 Partokens 吗？', faq3a: '产品会明确标注本地会话。最近聊天记录默认存储在当前浏览器，Partokens 不保存这些会话记录。', faq4q: '现有 OpenAI SDK 可以继续使用吗？', faq4a: '可以使用兼容接口。创建 API 密钥后，按使用文档替换接入地址与密钥即可。', faq5q: '如何查看余额和每次调用？', faq5a: '控制台固定展示余额与用量；数据看板和使用日志提供更完整的请求记录。',
  footerBody: '把对话、创作和账户管理放进一个清楚的工作区。', product: '产品', resources: '资源', legal: '条款', contact: '联系', playgroundLink: '对话游乐场', studioLink: '生图工作台', statusLink: '服务状态', userAgreement: '用户协议', serviceAgreement: '服务协议', privacyPolicy: '隐私政策', emailSupport: '邮件支持', telegramSupport: 'Telegram 支持', copyright: 'Partokens. All rights reserved.',
...
const ru: Copy = {
  ...
  heroEyebrow: 'ДЛЯ ЛЮБОЙ РАБОТЫ С ИИ', heroTitle: 'Ваше пространство ИИ: от диалога до творчества.', heroBody: 'Общайтесь, создавайте изображения, управляйте API-ключами и отслеживайте использование в одном аккаунте.', openConsole: 'Начать сейчас', viewDocs: 'Открыть документацию', previewWorkspace: 'Пространство чата', previewNewChat: 'Новый чат', previewQuestion: 'Составь список проверок перед запуском продукта.', previewAnswer: 'Конечно. Разделю его на проверку перед релизом, запуск и подготовку отката.', previewPrompt: 'Введите сообщение, чтобы начать', localConversation: 'История хранится только на этом устройстве',
  ...
  return <>
    <header className="public-header">
      ...
      <nav className="public-nav" aria-label="Primary">
        <button onClick={() => go('docs')}>{t.docs}</button><button onClick={() => go('about')}>{t.about}</button>
      </nav>
...
function HomeScreen({ t, locale, theme, onLocale, onTheme, go, online, version, onNotice }: CommonScreenProps & { online: boolean | null; version?: string }) {
  ...
  const faqs = [[t.faq1q, t.faq1a], [t.faq3q, t.faq3a], [t.faq4q, t.faq4a], [t.faq5q, t.faq5a]]
  return <div className="home-screen">
    <PublicHeader {...{ t, locale, theme, onLocale, onTheme, go, onNotice }} />
    <main>
      <section className="hero-section">
        <div className="hero-copy"><span className="section-kicker">{t.heroEyebrow}</span><h1>Partokens</h1><h2>{t.heroTitle}</h2><p>{t.heroBody}</p><div className="hero-actions"><button className="primary-button" onClick={() => go('console')}>{t.openConsole}<ArrowRight size={17} /></button><button className="secondary-button" onClick={() => go('docs')}><BookOpen size={17} />{t.viewDocs}</button></div></div>
