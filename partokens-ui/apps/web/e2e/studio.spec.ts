...
test('Studio uses existing user-created keys from the Image group only', async ({ page }) => {
  ...
  const requestsBeforeReturn = tokenListRequests
  await page.getByRole('link', { name: 'Image studio' }).click()
  await expect.poll(() => tokenListRequests).toBeGreaterThan(requestsBeforeReturn)
...
test('Studio sends a custom image size and ten requested images', async ({ page }) => {
  ...
  await page.getByLabel('Height').pressSequentially('900')
  await expect(page.getByLabel('Width')).toHaveValue('1600')
  await expect(page.getByLabel('Height')).toHaveValue('900')
  ...
  expect(generationPayload).toMatchObject({ size: '1600x900', n: 10 })
