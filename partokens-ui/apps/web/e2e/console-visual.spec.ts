...
for (const viewport of viewports) {
  for (const theme of ['light', 'dark'] as const) {
    ...
    test(`Canonical Console Analytics production ${theme} ${viewport.name}`, async ({ page }) => {
      ...
      const selectedMeasure = page.getByRole('tab', { name: 'Requests' })
      const unselectedMeasure = page.getByRole('tab', { name: 'Tokens' })
      expect(await selectedMeasure.evaluate((element) => getComputedStyle(element).borderTopColor)).not.toBe('rgba(0, 0, 0, 0)')
      expect(await unselectedMeasure.evaluate((element) => getComputedStyle(element).borderTopColor)).toBe('rgba(0, 0, 0, 0)')
      await expect(page.locator('html')).toHaveAttribute('data-theme', theme)
      await expectNoOverflow(page)
    ...
    test(`Canonical Console API Keys production ${theme} ${viewport.name}`, async ({ page }) => {
      ...
      await expect(page.getByRole('heading', { name: 'API keys' })).toBeVisible()
      await expect(page.getByRole('combobox', { name: 'Permission filter unavailable' })).toHaveCount(0)
      await expect(page.locator('html')).toHaveAttribute('data-theme', theme)
      await expectNoOverflow(page)
...
test('Canonical Console Analytics removes secondary actions and keeps both selectors consistent', async ({ page }) => {
  ...
  expect(await page.getByRole('tab', { name: 'Requests' }).evaluate((element) => getComputedStyle(element).borderTopColor)).not.toBe('rgba(0, 0, 0, 0)')
  await page.getByRole('tab', { name: 'Models' }).click()
  expect(await page.getByRole('tab', { name: 'Models' }).evaluate((element) => getComputedStyle(element).borderTopColor)).not.toBe('rgba(0, 0, 0, 0)')
  expect(await page.getByRole('tab', { name: 'Trend' }).evaluate((element) => getComputedStyle(element).borderTopColor)).toBe('rgba(0, 0, 0, 0)')
  await page.screenshot({ path: `${output}/analytics-production-selectors-1440.png`, fullPage: true })
})

test('Canonical Console Analytics loading and unavailable states preserve the composition', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 })
  await primeUserSession(page)
  await installMockApi(page)
...
test('Canonical Console API Keys uses desktop dialog, mobile sheet, confirmation, and restores focus', async ({ page }) => {
  ...
  await page.getByRole('combobox', { name: 'Expiration time' }).click()
  await page.getByRole('option', { name: 'Custom time' }).click()
  const calendar = page.locator('[data-custom-expiration-calendar]')
  await expect(calendar).toBeVisible()
  await expect(page.locator('[data-slot="popover-content"][aria-label="Custom time"]')).toHaveCSS('width', '250px')
  await expect(calendar.locator('[data-slot="calendar"]')).toHaveCSS('width', '248px')
  await expect(calendar.locator('.rdp-month_grid')).toHaveCSS('width', '224px')
  await expect(calendar.locator('.rdp-weekday').first()).toHaveCSS('text-align', 'center')
  await expect(calendar.locator('.rdp-weekday').first()).toHaveCSS('border-bottom-width', '0px')
  await expect(calendar.locator('.rdp-day').first()).toHaveCSS('width', '32px')
  await expect(calendar.locator('.rdp-day').first()).toHaveCSS('height', '32px')
  await page.screenshot({ path: `${output}/keys-production-calendar-1440.png` })
  await calendar.locator('button[data-day]').filter({ hasText: /^\d{1,2}$/ }).last().click()
  await page.keyboard.press('Escape')
...
test('Canonical Console Usage Logs uses desktop dialog, mobile sheet, and restores focus', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  await primeUserSession(page)
  await installMockApi(page)
  await page.goto('/en/console/usage-logs')
