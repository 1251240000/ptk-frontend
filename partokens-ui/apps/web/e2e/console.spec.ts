...
test('canonical console API keys uses the token CRUD, status, batch, and reveal contracts without retaining secrets', async ({ page }) => {
  ...
  await page.getByRole('option', { name: 'Image 1.5x Ratio', exact: true }).click()
  await expect(page.getByLabel('Group')).toContainText('1.5x Ratio')
  ...
  await page.getByLabel('Group').click()
  await expect(page.getByRole('option', { name: 'default', exact: true })).toHaveCount(0)
  const imageGroupOption = page.getByRole('option', { name: 'Image 1.5x Ratio', exact: true })
  await expect(imageGroupOption).toBeVisible()
  const lightRatioBadge = imageGroupOption.locator('[data-slot="badge"]')
  await expect(lightRatioBadge).toContainText('1.5x Ratio')
  const lightRatioBadgeStyle = await lightRatioBadge.evaluate((element) => {
    const style = getComputedStyle(element)
    return { color: style.color, borderColor: style.borderColor }
  })
  expect(lightRatioBadgeStyle.color).not.toBe('rgba(0, 0, 0, 0)')
  expect(lightRatioBadgeStyle.borderColor).not.toBe('rgba(0, 0, 0, 0)')
  await imageGroupOption.click()
  await expect(page.getByLabel('Group')).toContainText('1.5x Ratio')
...
for (const width of [390, 320]) {
  test(`canonical console API keys stays responsive at ${width}px with locale, theme, and focus intact`, async ({ page }) => {
    ...
    const groupSelect = page.getByLabel('分组')
    await groupSelect.click()
    const imageGroupOption = page.getByRole('option', { name: 'Image 1.5x 倍率', exact: true })
    await expect(imageGroupOption.locator('[data-slot="badge"]')).toContainText('1.5x 倍率')
    await imageGroupOption.click()
    await expect(groupSelect).toContainText('1.5x 倍率')
