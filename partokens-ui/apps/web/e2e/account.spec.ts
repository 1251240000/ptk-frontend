...
test('Profile keeps identity fields in the requested order and replaces daily check-in with vertical account status', async ({ page }) => {
  await primeUserSession(page)
  await installMockApi(page)
  await page.goto('/en/console/profile')

  await expect(page.getByText('Daily check-in')).toHaveCount(0)
  const profileSection = page.getByRole('heading', { name: 'Account profile' }).locator('xpath=ancestor::form[1]')
  const statusSection = page.getByRole('heading', { name: 'Account status' }).locator('xpath=ancestor::section[1]')
  const fieldLabels = await profileSection.locator('fieldset label').allTextContents()
  expect(fieldLabels).toEqual(['Username', 'Email', 'Display name', 'Interface language'])

  const profileBox = await profileSection.boundingBox()
  const statusBox = await statusSection.boundingBox()
  if (!profileBox || !statusBox) throw new Error('Profile layout could not be measured')
  expect(statusBox.x).toBeGreaterThan(profileBox.x + profileBox.width - 1)
  const statusRows = statusSection.locator('> div.divide-y > div')
  await expect(statusRows).toHaveCount(3)
  const rowBoxes = await statusRows.evaluateAll((rows) => rows.map((row) => row.getBoundingClientRect()))
  expect(rowBoxes[1].top).toBeGreaterThanOrEqual(rowBoxes[0].bottom - 1)
  expect(rowBoxes[2].top).toBeGreaterThanOrEqual(rowBoxes[1].bottom - 1)
})
...
test('redacted notification secrets remain configured and are omitted when unchanged', async ({ page }) => {
  ...
  await expect(page.getByText('Requests may use models whose price has not been set')).toHaveCount(0)
  await expect(page.getByText('Store source IP addresses in your private usage logs')).toHaveCount(0)
...
test('Account security overlays complete the protected credential workflows', async ({ page }) => {
  ...
  const regenerateDialog = page.getByRole('dialog', { name: 'Regenerate backup codes' })
  const authenticatorCode = regenerateDialog.getByLabel('Authenticator code')
  const regenerateButton = regenerateDialog.getByRole('button', { name: 'Regenerate backup codes' })
  await expect(authenticatorCode).toBeVisible()
  await expect(regenerateDialog.getByLabel('Authenticator or backup code')).toHaveCount(0)
  await authenticatorCode.fill('BACKUP-1001')
  await expect(authenticatorCode).toHaveValue('')
  ...
  const tokenResultDialog = page.getByRole('dialog', { name: 'System access token' })
  await expect(tokenResultDialog.getByText('fixture-transient-system-token')).toBeVisible()
  await expect(tokenResultDialog.getByRole('button', { name: 'Copy' })).toHaveCount(2)
  await page.evaluate(() => Object.defineProperty(navigator.clipboard, 'writeText', { configurable: true, value: async () => undefined }))
  await tokenResultDialog.getByRole('button', { name: 'Copy' }).last().click()
  await expect(page.getByText('Access token copied.', { exact: true })).toBeVisible()
  await expect(tokenResultDialog).toBeVisible()
  await tokenResultDialog.getByRole('button', { name: 'Done' }).click()
  await expect(page.getByText('fixture-transient-system-token')).toHaveCount(0)
