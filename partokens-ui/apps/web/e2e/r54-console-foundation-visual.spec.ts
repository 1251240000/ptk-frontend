...
const output = '../../dogfood-output/r54-console-foundation-rebuild/screenshots'
const viewports = [
...
for (const viewport of viewports) {
  for (const theme of ['light', 'dark'] as const) {
    test(`R54 Overview ${theme} ${viewport.name}`, async ({ page }) => {
    ...
    test(`R54 Overview design ${theme} ${viewport.name}`, async ({ page }) => {
      await page.setViewportSize(viewport)
      await setTheme(page, theme)
    ...
    test(`R54 Analytics design ${theme} ${viewport.name}`, async ({ page }) => {
      await page.setViewportSize(viewport)
      await setTheme(page, theme)
    ...
    test(`R54 Analytics production ${theme} ${viewport.name}`, async ({ page }) => {
      await page.setViewportSize(viewport)
      await primeUserSession(page)
      await setTheme(page, theme)
      await installMockApi(page)
    ...
    test(`R54 API Keys design ${theme} ${viewport.name}`, async ({ page }) => {
      await page.setViewportSize(viewport)
      await setTheme(page, theme)
    ...
    test(`R54 API Keys production ${theme} ${viewport.name}`, async ({ page }) => {
      await page.setViewportSize(viewport)
      await primeUserSession(page)
      await setTheme(page, theme)
      await installMockApi(page)
    ...
    test(`R54 Usage Logs design ${theme} ${viewport.name}`, async ({ page }) => {
      await page.setViewportSize(viewport)
      await setTheme(page, theme)
    ...
    test(`R54 Usage Logs production ${theme} ${viewport.name}`, async ({ page }) => {
      await page.setViewportSize(viewport)
      await primeUserSession(page)
      await setTheme(page, theme)
      await installMockApi(page)
...
test('R54 Overview overlay and mobile navigation restore focus', async ({ page }) => {
...
test('R54 Overview loading and partial states preserve the composition', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 })
  await primeUserSession(page)
  await installMockApi(page)
...
test('R54 Analytics details use dialog and sheet with focus restoration', async ({ page }) => {
...
test('R54 Analytics loading and contract states preserve the composition', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 })
  await primeUserSession(page)
  await installMockApi(page)
...
test('R54 API Keys uses desktop dialog, mobile sheet, confirmation, and restores focus', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  await primeUserSession(page)
  await installMockApi(page)
  await page.goto('/en/console-foundation/keys')
...
test('R54 API Keys loading, partial, and contract states preserve the list surface', async ({ page }) => {
...
test('R54 Usage Logs uses desktop dialog, mobile sheet, disabled export, and restores focus', async ({ page }) => {
  await page.setViewportSize({ width: 1440, height: 1000 })
  await primeUserSession(page)
  await installMockApi(page)
  await page.goto('/en/console-foundation/logs')
...
test('R54 Usage Logs loading, partial, and contract states preserve the design-lab composition', async ({ page }) => {
  let state: 'partial' | 'contract' = 'partial'
  await page.setViewportSize({ width: 390, height: 844 })
  await primeUserSession(page)
  await installMockApi(page)
