...
test('password login completes the required two-factor route before entering the console', async ({ page }) => {
  ...
  await page.waitForURL('**/en/console/overview')
  expect(verification).toEqual({ code: '123456' })
})
...
test('API key lifecycle covers create, edit, disable, reveal, and delete', async ({ page }) => {
  ...
  await expect(page.getByRole('dialog', { name: 'Configure your new key' })).toBeVisible()
...
test('usage-log filters preserve self scope and open a redacted detail drawer', async ({ page }) => {
  ...
  await page.getByLabel('Request ID').fill('req_fixture')
  ...
  await expect(page.getByText('Fixture completion request')).toBeVisible()
  await expect(page.getByText('Billing calculation')).toBeVisible()
})
...
test('overview uses live fixture state and model filters change the visible catalog', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name: 'Inspect logs' })).toBeVisible()
  await expect(page.locator('.endpoint-value')).toHaveText(`${new URL(page.url()).origin}/v1`)
