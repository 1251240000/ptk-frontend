...
import { locales, playgroundTranslationRows, resources } from '@partokens/i18n'

describe('Playground translations', () => {
  it('provides every new Playground key in all seven locales', () => {
    for (const row of playgroundTranslationRows) {
  ...
  it('resolves every static Playground source key', () => {
    const sourceRoot = resolve(dirname(fileURLToPath(import.meta.url)), '..')
    const files = [
      resolve(sourceRoot, 'pages/workbench-pages.tsx'),
      resolve(sourceRoot, 'features/playground/playground-ui.tsx'),
    ]
