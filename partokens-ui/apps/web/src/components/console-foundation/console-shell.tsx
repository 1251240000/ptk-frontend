...
type ConsolePage = 'overview' | 'analytics' | 'keys' | 'logs'

type NavigationItem = {
  label: string
  icon: typeof KeyRound
  page?: ConsolePage
  canonicalPath?: '/console/playground' | '/console/studio' | '/console/wallet' | '/console/profile'
  unavailable?: boolean
}
...
const consoleNavigation: Array<{ label: string; items: NavigationItem[] }> = [
  {
    ...
    items: [
      ...
      { label: 'Usage logs', page: 'logs', icon: ReceiptText },
    ],
  },
  {
    label: 'Workspace',
    items: [
      { label: 'Playground', canonicalPath: '/console/playground', icon: MessageSquare },
      { label: 'Image studio', canonicalPath: '/console/studio', icon: ImageIcon },
    ],
  },
  {
    label: 'Account',
    items: [
      { label: 'Wallet', canonicalPath: '/console/wallet', icon: WalletCards },
      { label: 'Profile', canonicalPath: '/console/profile', icon: CircleUserRound },
...
function currentPage(pathname: string): ConsolePage {
        ...
        <DropdownMenu>
          ...
          >
            ...
            <DropdownMenuItem className="gap-2 p-2" asChild>
              <Link to="/$locale/console-foundation/overview" params={{ locale }} onClick={closeMobileNavigation}>
              ...
              <SidebarMenuItem key={item.label}>
                {item.page ? (
                  <SidebarMenuButton asChild isActive={isActive} tooltip={t(item.label)}>
                    <Link
                      to={`/$locale/console-foundation/${item.page}`}
                      params={{ locale }}
                  ...
                  <SidebarMenuButton asChild tooltip={t(item.label)}>
                    <Link to={`/$locale${item.canonicalPath}`} params={{ locale }} onClick={closeMobileNavigation}>
                      {content}
                    </Link>
                  </SidebarMenuButton>
                ) : (
          ...
          >
            ...
            <DropdownMenuGroup>
              <DropdownMenuItem asChild>
                <Link to="/$locale/console/profile" params={{ locale }}><CircleUserRound />{t('Profile')}</Link>
              </DropdownMenuItem>
...
export function ConsoleShell() {
  ...
  useEffect(() => {
    document.body.classList.add('console-foundation-portal')
    return () => document.body.classList.remove('console-foundation-portal')
  }, [])

  return (
    <div className="console-foundation" lang={document.documentElement.lang}>
      <SidebarProvider
      ...
      >
        ...
        <SidebarInset className="@container/content min-w-0">
          ...
          <main className="w-full px-4 py-6 sm:px-6" tabIndex={-1}>
