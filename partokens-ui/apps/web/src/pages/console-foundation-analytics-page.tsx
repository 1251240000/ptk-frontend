...
class AnalyticsContractError extends Error {
  constructor() {
...
class SafeAnalyticsError extends Error {
  status?: number

  constructor(status?: number) {
    super('Analytics data is unavailable.')
    this.name = 'SafeAnalyticsError'
    this.status = status
  }
}
...
async function safeRequest<T>(request: () => Promise<unknown>, project: (value: unknown) => T): Promise<T> {
  try {
    ...
    if (error instanceof AnalyticsContractError) throw error
    const status = (error as { response?: { status?: number } } | null)?.response?.status
    throw new SafeAnalyticsError(status)
  }
}

function errorMessage(error: unknown): string {
  if (error instanceof AnalyticsContractError) return 'The analytics response contract is incomplete.'
  const status = error instanceof SafeAnalyticsError ? error.status : undefined
  if (status === 401) return 'Sign in again to load analytics data.'
  if (status === 403) return 'Your account cannot access this analytics data.'
  return 'The aggregate usage request failed. Try again to reload the current period.'
}
...
async function loadUsage(range: AnalyticsRange, granularity: AnalyticsGranularity): Promise<UsageWindow> {
  const default_time = requestGranularity(granularity)
  const [current, previous] = await Promise.all([
    safeRequest(() => getQuotaData({ ...requestBounds(range), default_time }), projectUsage),
    safeRequest(() => getQuotaData({ ...requestBounds(range, true), default_time }), projectUsage),
  ])
...
function measureLabel(measure: AnalyticsMeasure) {
  if (measure === 'quota') return 'Cost'
  if (measure === 'tokens') return 'Tokens'
  return 'Requests'
}
...
function buildFlows(rows: SafeFlowRow[]): FlowTotal[] {
  const flows = new Map<string, FlowTotal>()
  for (const row of rows) {
    const tokenName = row.tokenName ?? (row.tokenId ? `Deleted key #${row.tokenId}` : 'Deleted key')
    const group = row.group ?? 'Unavailable'
    const modelName = row.modelName ?? 'Unavailable'
...
function StatePanel(props: { kind: 'error' | 'empty' | 'contract'; message: string; onRetry?: () => void }) {
  return (
    <section role={props.kind === 'empty' ? undefined : 'alert'} className="flex min-h-96 flex-col items-center justify-center rounded-md border px-6 py-12 text-center">
      ...
      <h2 className="text-sm font-semibold">{props.kind === 'empty' ? 'No usage in this period' : props.kind === 'contract' ? 'Analytics contract is incomplete' : 'Analytics data is unavailable'}</h2>
      <p className="mt-1 max-w-md text-sm text-muted-foreground">{props.message}</p>
      {props.onRetry ? <Button variant="outline" size="sm" className="mt-4" onClick={props.onRetry}><RefreshCw />Try again</Button> : null}
    </section>
  )
}
...
function MetricSummary(props: { current: MetricTotals; previous: MetricTotals; locale: AppLocale; currentRows: SafeUsageRow[]; previousRows: SafeUsageRow[] }) {
  const items = (['requests', 'tokens', 'quota'] as const).map((metric) => ({
    label: measureLabel(metric),
    value: metricComplete(props.currentRows, metric) ? formatMeasure(props.current[metric], metric, props.locale) : '—',
    note: metricComplete(props.previousRows, metric) ? `${formatMeasure(props.previous[metric], metric, props.locale)} previous` : 'Previous unavailable',
  }))
  items.push({ label: 'Success rate', value: '—', note: 'Contract unavailable' })
  return (
  ...
  return (
    <div className="grid lg:grid-cols-[minmax(0,3fr)_minmax(220px,1fr)]">
      <figure className="min-w-0 p-4 sm:p-6">
        <figcaption className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-xs font-medium text-muted-foreground">{measureLabel(props.measure)}</p><p className="mt-1 font-mono text-2xl font-semibold">{formatMeasure(current, props.measure, props.locale, true)}</p></div><Badge variant="secondary" className="font-mono">{change == null ? 'Previous unavailable' : `${change >= 0 ? '+' : ''}${change.toFixed(1)}%`}</Badge></figcaption>
        <div className="relative mt-6 h-64">
          ...
          <div className="relative z-10 grid h-full gap-1 sm:gap-2" style={{ gridTemplateColumns: `repeat(${props.points.length}, minmax(0, 1fr))` }}>
            {props.points.map((point, index) => {
              ...
              return <div key={point.timestamp} className="grid min-w-0 grid-rows-[minmax(0,1fr)_1.25rem] gap-2"><Tooltip><TooltipTrigger asChild><div tabIndex={0} role="img" aria-label={`${point.label}: ${formatMeasure(present, props.measure, props.locale)} current, ${formatMeasure(past, props.measure, props.locale)} previous`} className="flex h-full min-w-0 items-end justify-center gap-px rounded-sm focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:outline-none sm:gap-1"><span className="block w-[35%] rounded-t-sm bg-muted-foreground/25" style={{ height: `${Math.max(3, past / maximum * 100)}%` }} /><span className="block w-[35%] rounded-t-sm bg-chart-2" style={{ height: `${Math.max(3, present / maximum * 100)}%` }} /></div></TooltipTrigger><TooltipContent side="top"><p className="font-medium">{point.label}</p><p>Current: {formatMeasure(present, props.measure, props.locale)}</p><p>Previous: {formatMeasure(past, props.measure, props.locale)}</p></TooltipContent></Tooltip><span className="truncate text-center text-[10px] text-muted-foreground">{showLabel ? point.shortLabel : ''}</span></div>
            })}
          </div>
        </div>
      </figure>
      <aside className="border-t p-4 lg:border-t-0 lg:border-s sm:p-6"><p className="text-sm font-medium">Period comparison</p><p className="mt-1 text-sm text-muted-foreground">Usage aggregates for the selected measure and the immediately preceding period.</p><dl className="mt-6 space-y-4 text-sm"><div className="flex items-center justify-between gap-4"><dt className="flex items-center gap-2 text-muted-foreground"><span className="size-2 rounded-sm bg-chart-2" />Current period</dt><dd className="font-mono font-medium">{formatMeasure(current, props.measure, props.locale, true)}</dd></div><div className="flex items-center justify-between gap-4"><dt className="flex items-center gap-2 text-muted-foreground"><span className="size-2 rounded-sm bg-muted-foreground/25" />Previous period</dt><dd className="font-mono font-medium">{formatMeasure(previous, props.measure, props.locale, true)}</dd></div></dl></aside>
    </div>
  )
}
...
function ModelRanking(props: { models: ModelTotal[]; measure: AnalyticsMeasure; locale: AppLocale; requestsReady: boolean; quotaReady: boolean }) {
  ...
  return <div><div className="hidden md:block"><Table><TableHeader><TableRow className="hover:bg-transparent"><TableHead className="ps-4">Model</TableHead><TableHead>Share</TableHead><TableHead>Requests</TableHead><TableHead>Cost</TableHead><TableHead>Success</TableHead><TableHead>Median latency</TableHead></TableRow></TableHeader><TableBody>{sorted.map((model) => { const share = total ? measureValue(model, props.measure) / total : 0; return <TableRow key={model.name}><TableCell className="max-w-64 truncate ps-4 font-medium">{model.name}</TableCell><TableCell><div className="flex min-w-36 items-center gap-3"><span className="h-1.5 flex-1 overflow-hidden rounded-full bg-muted"><span className="block h-full bg-chart-2" style={{ width: `${share * 100}%` }} /></span><span className="w-10 text-end font-mono text-xs">{new Intl.NumberFormat(props.locale, { style: 'percent', maximumFractionDigits: 1 }).format(share)}</span></div></TableCell><TableCell className="font-mono text-xs">{props.requestsReady ? formatInteger(model.requests, props.locale) : '—'}</TableCell><TableCell className="font-mono text-xs">{props.quotaReady ? formatQuota(model.quota, props.locale) : '—'}</TableCell><TableCell className="text-muted-foreground">Unavailable</TableCell><TableCell className="text-muted-foreground">Unavailable</TableCell></TableRow> })}</TableBody></Table></div><div className="divide-y md:hidden">{sorted.map((model, index) => { const share = total ? measureValue(model, props.measure) / total : 0; return <article key={model.name} className="space-y-4 p-4"><div className="flex min-w-0 items-center justify-between gap-3"><div className="min-w-0"><p className="text-xs text-muted-foreground">#{index + 1}</p><h3 className="break-words text-sm font-medium">{model.name}</h3></div><Badge variant="secondary">{new Intl.NumberFormat(props.locale, { style: 'percent', maximumFractionDigits: 1 }).format(share)}</Badge></div><div className="h-1.5 overflow-hidden rounded-full bg-muted"><span className="block h-full bg-chart-2" style={{ width: `${share * 100}%` }} /></div><dl className="grid grid-cols-2 gap-3 text-sm"><div><dt className="text-xs text-muted-foreground">Requests</dt><dd className="mt-1 font-mono text-xs">{props.requestsReady ? formatInteger(model.requests, props.locale) : '—'}</dd></div><div><dt className="text-xs text-muted-foreground">Cost</dt><dd className="mt-1 font-mono text-xs">{props.quotaReady ? formatQuota(model.quota, props.locale) : '—'}</dd></div><div><dt className="text-xs text-muted-foreground">Success</dt><dd className="mt-1 text-muted-foreground">Unavailable</dd></div><div><dt className="text-xs text-muted-foreground">Median latency</dt><dd className="mt-1 text-muted-foreground">Unavailable</dd></div></dl></article> })}</div></div>
}
...
function RequestFlow(props: { flows: FlowTotal[]; measure: AnalyticsMeasure; locale: AppLocale }) {
  ...
  return <div><div className="hidden md:block"><Table><TableHeader><TableRow className="hover:bg-transparent"><TableHead className="ps-4">API key</TableHead><TableHead>Group</TableHead><TableHead>Model route</TableHead><TableHead>{measureLabel(props.measure)}</TableHead><TableHead>Share</TableHead></TableRow></TableHeader><TableBody>{sorted.map((flow, index) => { const share = total ? measureValue(flow, props.measure) / total : 0; return <TableRow key={`${flow.tokenName}:${flow.modelName}:${index}`}><TableCell className="max-w-56 ps-4 font-medium"><span className="flex min-w-0 items-center gap-2"><KeyRound className="size-4 shrink-0 text-muted-foreground" /><span className="truncate">{flow.tokenName}</span></span></TableCell><TableCell><Badge variant="outline">{flow.group}</Badge></TableCell><TableCell className="max-w-64"><span className="flex min-w-0 items-center gap-2"><Bot className="size-4 shrink-0 text-muted-foreground" /><span className="truncate">{flow.modelName}</span></span></TableCell><TableCell className="font-mono text-xs">{formatMeasure(measureValue(flow, props.measure), props.measure, props.locale)}</TableCell><TableCell className="font-mono text-xs">{new Intl.NumberFormat(props.locale, { style: 'percent', maximumFractionDigits: 1 }).format(share)}</TableCell></TableRow> })}</TableBody></Table></div><div className="divide-y md:hidden">{sorted.map((flow, index) => { const share = total ? measureValue(flow, props.measure) / total : 0; return <article key={`${flow.tokenName}:${flow.modelName}:${index}`} className="space-y-3 p-4"><div className="flex min-w-0 items-start gap-2 text-sm font-medium"><KeyRound className="mt-0.5 size-4 shrink-0 text-muted-foreground" /><span className="min-w-0 break-words">{flow.tokenName}</span><Badge variant="outline" className="ms-auto shrink-0">{flow.group}</Badge></div><div className="flex min-w-0 items-start gap-2 text-sm"><Route className="mt-0.5 size-4 shrink-0 text-muted-foreground" /><span className="min-w-0 break-words">{flow.modelName}</span></div><div className="flex items-center justify-between gap-3 text-xs text-muted-foreground"><span className="font-mono">{formatMeasure(measureValue(flow, props.measure), props.measure, props.locale)}</span><span className="shrink-0 font-mono">{new Intl.NumberFormat(props.locale, { style: 'percent', maximumFractionDigits: 1 }).format(share)}</span></div></article> })}</div></div>
}

function DataRows(props: { points: TrendPoint[]; locale: AppLocale }) {
  return <><div className="hidden sm:block"><div className="max-h-[55vh] overflow-auto rounded-md border"><Table><TableHeader className="sticky top-0 bg-background"><TableRow className="hover:bg-transparent"><TableHead>Period</TableHead><TableHead>Requests</TableHead><TableHead>Tokens</TableHead><TableHead>Cost</TableHead><TableHead>Previous requests</TableHead></TableRow></TableHeader><TableBody>{props.points.map((point) => <TableRow key={point.timestamp}><TableCell className="font-medium">{point.label}</TableCell><TableCell className="font-mono text-xs">{formatInteger(point.requests, props.locale)}</TableCell><TableCell className="font-mono text-xs">{formatInteger(point.tokens, props.locale)}</TableCell><TableCell className="font-mono text-xs">{formatQuota(point.quota, props.locale)}</TableCell><TableCell className="font-mono text-xs">{formatInteger(point.previous.requests, props.locale)}</TableCell></TableRow>)}</TableBody></Table></div></div><div className="divide-y rounded-md border sm:hidden">{props.points.map((point) => <article key={point.timestamp} className="space-y-3 p-3"><h3 className="text-sm font-medium">{point.label}</h3><dl className="grid grid-cols-2 gap-3 text-sm"><div><dt className="text-xs text-muted-foreground">Requests</dt><dd className="mt-1 font-mono text-xs">{formatInteger(point.requests, props.locale)}</dd></div><div><dt className="text-xs text-muted-foreground">Tokens</dt><dd className="mt-1 font-mono text-xs">{formatInteger(point.tokens, props.locale)}</dd></div><div><dt className="text-xs text-muted-foreground">Cost</dt><dd className="mt-1 font-mono text-xs">{formatQuota(point.quota, props.locale)}</dd></div><div><dt className="text-xs text-muted-foreground">Previous requests</dt><dd className="mt-1 font-mono text-xs">{formatInteger(point.previous.requests, props.locale)}</dd></div></dl></article>)}</div></>
}
...
function AnalyticsDataDetails(props: { points: TrendPoint[]; period: string; locale: AppLocale; onClose: () => void; onClosedAutoFocus: () => void }) {
  ...
  if (isMobile) return <Sheet open onOpenChange={(open) => !open && props.onClose()}><SheetContent className="w-full sm:max-w-md" onCloseAutoFocus={closeAutoFocus}><SheetHeader><SheetTitle>Aggregated usage data</SheetTitle><SheetDescription>{props.period}</SheetDescription></SheetHeader><div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4"><DataRows points={props.points} locale={props.locale} /></div><SheetFooter className="border-t"><Button variant="outline" onClick={props.onClose}>Close</Button></SheetFooter></SheetContent></Sheet>
  return <Dialog open onOpenChange={(open) => !open && props.onClose()}><DialogContent className="sm:max-w-3xl" onCloseAutoFocus={closeAutoFocus}><DialogHeader><DialogTitle>Aggregated usage data</DialogTitle><DialogDescription>{props.period}</DialogDescription></DialogHeader><DataRows points={props.points} locale={props.locale} /><DialogFooter><Button variant="outline" onClick={props.onClose}>Close</Button></DialogFooter></DialogContent></Dialog>
}

function SupportingSummary(props: { modelCount: number; requestCount?: number; locale: AppLocale }) {
  return <div className="grid gap-4 lg:grid-cols-2"><section aria-labelledby="analytics-model-health" className="overflow-hidden rounded-md border"><header className="border-b p-4"><h2 id="analytics-model-health" className="text-sm font-semibold">Model health</h2><p className="mt-1 text-sm text-muted-foreground">Availability and latency across models with recorded usage.</p></header><dl className="grid grid-cols-3 divide-x p-4 text-center"><div><dt className="text-xs text-muted-foreground">Models</dt><dd className="mt-1 font-mono text-lg font-semibold">{formatInteger(props.modelCount, props.locale)}</dd></div><div><dt className="text-xs text-muted-foreground">Success</dt><dd className="mt-1 text-sm text-muted-foreground">Unavailable</dd></div><div><dt className="text-xs text-muted-foreground">Median</dt><dd className="mt-1 text-sm text-muted-foreground">Unavailable</dd></div></dl></section><section aria-labelledby="analytics-routes" className="overflow-hidden rounded-md border"><header className="border-b p-4"><h2 id="analytics-routes" className="text-sm font-semibold">Request routes</h2><p className="mt-1 text-sm text-muted-foreground">{props.requestCount == null ? 'Request count is available; endpoint dimensions are not.' : `${formatInteger(props.requestCount, props.locale)} requests; endpoint dimensions are not exposed.`}</p></header><div className="flex min-h-20 items-center px-4 text-sm text-muted-foreground"><AlertTriangle className="me-2 size-4 shrink-0" />Contract unavailable</div></section></div>
}
...
function exportAnalytics(points: TrendPoint[], format: 'csv' | 'json') {
  if (format === 'json') {
  ...
  downloadFile('partokens-analytics.csv', 'text/csv;charset=utf-8', [['Period', 'Requests', 'Tokens', 'Quota units', 'Previous requests', 'Previous tokens', 'Previous quota units'].map(escape).join(','), ...rows].join('\n'))
}

export function ConsoleFoundationAnalyticsPage() {
  ...
  const usage = useQuery({ queryKey: ['console-foundation', 'analytics', 'usage', appliedRange, appliedGranularity], queryFn: () => loadUsage(appliedRange, appliedGranularity), retry: false })
  const flow = useQuery({ queryKey: ['console-foundation', 'analytics', 'flow', appliedRange, appliedGranularity], queryFn: () => safeRequest(() => getFlowQuotaData({ ...requestBounds(appliedRange), default_time: requestGranularity(appliedGranularity) }), projectFlow), retry: false })
  ...
  const flows = useMemo(() => buildFlows(flowRows), [flowRows])
  ...
  const refresh = () => void Promise.all([usage.refetch(), flow.refetch()])
  ...
  return (
    <div className="flex flex-col gap-6 pb-8">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><h1 id="console-foundation-title" className="text-2xl font-bold tracking-tight">{t('Analytics')}</h1><p className="text-muted-foreground">{t('Inspect usage trends, model share, cost, and request routes.')}</p></div><div className="flex flex-wrap items-center gap-2"><Button variant="outline" disabled={refreshing} onClick={refresh}>{refreshing ? <LoaderCircle className="animate-spin" /> : <RefreshCw />}{refreshing ? t('Refreshing...') : t('Refresh')}</Button><DropdownMenu><DropdownMenuTrigger asChild><Button disabled={refreshing || !trend.length || usagePartial}><Download />{t('Export')}<ChevronDown className="ms-1" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-48"><DropdownMenuLabel>Aggregated usage</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuItem onSelect={() => exportAnalytics(trend, 'csv')}><Download />Export CSV</DropdownMenuItem><DropdownMenuItem onSelect={() => exportAnalytics(trend, 'json')}><Download />Export JSON</DropdownMenuItem></DropdownMenuContent></DropdownMenu><DropdownMenu><DropdownMenuTrigger asChild><Button variant="ghost" size="icon" aria-label="Analytics status"><MoreHorizontal /></Button></DropdownMenuTrigger><DropdownMenuContent align="end" className="w-56"><DropdownMenuLabel>Contract status</DropdownMenuLabel><DropdownMenuSeparator /><DropdownMenuItem disabled>Usage: {usage.isError ? 'unavailable' : usagePartial ? 'partial' : 'validated'}</DropdownMenuItem><DropdownMenuItem disabled>Flow: {flow.isError ? 'unavailable' : flowPartial ? 'partial' : 'validated'}</DropdownMenuItem><DropdownMenuItem disabled>Success and latency: unavailable</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div></div>

      <section aria-label="Analytics filters" className="flex flex-col gap-3 lg:flex-row lg:items-center"><div className="grid grid-cols-2 gap-3 sm:flex sm:flex-wrap"><Select value={range} onValueChange={changeRange} disabled={refreshing}><SelectTrigger className="w-full sm:w-44" aria-label="Time range"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="1">Last 24 hours</SelectItem><SelectItem value="7">Last 7 days</SelectItem><SelectItem value="30">Last 30 days</SelectItem><SelectItem value="90">Last 90 days</SelectItem></SelectContent></Select><Select value={granularity} onValueChange={(value) => setGranularity(value as AnalyticsGranularity)} disabled={refreshing}><SelectTrigger className="w-full sm:w-36" aria-label="Granularity"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="hour" disabled={range !== '1'}>Hourly</SelectItem><SelectItem value="day" disabled={range === '1' || range === '90'}>Daily</SelectItem><SelectItem value="week" disabled={range === '1' || range === '7'}>Weekly</SelectItem></SelectContent></Select></div><Tabs value={measure} onValueChange={(value) => setMeasure(value as AnalyticsMeasure)} className="min-w-0 sm:w-auto"><TabsList aria-label="Measure" className="grid w-full grid-cols-3 sm:w-auto"><TabsTrigger value="requests">Requests</TabsTrigger><TabsTrigger value="tokens">Tokens</TabsTrigger><TabsTrigger value="quota">Cost</TabsTrigger></TabsList></Tabs><Button variant="outline" disabled={!pendingFilters || refreshing} onClick={applyFilters}>Apply filters</Button><span className="text-xs text-muted-foreground lg:ms-auto">{lastUpdated ? `Updated at ${new Intl.DateTimeFormat(locale, { hour: '2-digit', minute: '2-digit' }).format(lastUpdated)}` : 'Waiting for analytics data'}</span></section>

      {initialLoading || refreshing ? <AnalyticsLoading /> : usage.isError ? <div className="space-y-3"><StatePanel kind="error" message={errorMessage(usage.error)} onRetry={() => void usage.refetch()} />{flow.isError ? <SecondaryError message={errorMessage(flow.error)} /> : null}</div> : usagePartial && currentRows.length === 0 ? <StatePanel kind="contract" message="Returned rows do not expose any complete usage metrics." /> : currentRows.length === 0 ? <StatePanel kind="empty" message="Requests will appear here after an API key or workspace starts using a model." /> : (
        <><MetricSummary current={currentTotals} previous={previousTotals} locale={locale} currentRows={currentRows} previousRows={previousRows} />{usagePartial || flowPartial || flow.isError ? <div role="status" className="flex items-start gap-2 rounded-md border bg-muted/20 p-3 text-sm text-muted-foreground"><AlertTriangle className="mt-0.5 size-4 shrink-0" /><p>{flow.isError ? 'Request flow is unavailable. Usage views remain usable.' : 'Some aggregate rows are incomplete. Only validated dimensions are shown.'}</p></div> : null}<Tabs value={view} onValueChange={(value) => setView(value as AnalyticsView)}><section aria-labelledby="analytics-workspace-title" className="overflow-hidden rounded-md border"><header className="flex flex-col gap-4 border-b p-4 md:flex-row md:items-center md:justify-between"><div className="min-w-0"><h2 id="analytics-workspace-title" className="text-sm font-semibold">{viewTitles[view][0]}</h2><p className="mt-1 text-sm text-muted-foreground">{viewTitles[view][1]}</p></div><div className="flex flex-col gap-2 sm:flex-row sm:items-center"><TabsList className="grid w-full grid-cols-3 sm:w-auto"><TabsTrigger value="trend">Trend</TabsTrigger><TabsTrigger value="ranking">Models</TabsTrigger><TabsTrigger value="flow">Routes</TabsTrigger></TabsList><Button ref={detailTriggerRef} variant="outline" size="sm" disabled={!trend.length || usagePartial} onClick={openDetails}>View data</Button></div></header><TabsContent value="trend" className="mt-0">{!currentMeasureReady ? <StatePanel kind="contract" message={`Returned rows do not consistently expose ${measure} as a numeric field.`} /> : !trend.length ? <StatePanel kind="contract" message="Returned rows do not expose valid aggregate timestamps." /> : <TrendChart points={trend} measure={measure} locale={locale} />}</TabsContent><TabsContent value="ranking" className="mt-0">{!currentMeasureReady ? <StatePanel kind="contract" message={`Returned rows do not consistently expose ${measure} as a numeric field.`} /> : !ranking.length ? <StatePanel kind="contract" message="Returned rows do not expose validated model names." /> : <ModelRanking models={ranking} measure={measure} locale={locale} requestsReady={metricComplete(currentRows, 'requests')} quotaReady={metricComplete(currentRows, 'quota')} />}</TabsContent><TabsContent value="flow" className="mt-0">{flow.isError ? <StatePanel kind="error" message={errorMessage(flow.error)} onRetry={() => void flow.refetch()} /> : flowPartial && !flowRows.length ? <StatePanel kind="contract" message="Returned flow rows do not expose complete key, model, and usage dimensions." /> : !flowRows.length ? <StatePanel kind="empty" message="Key and model flow will appear after usage is recorded." /> : !flowMeasureReady ? <StatePanel kind="contract" message={`Returned flow rows do not consistently expose ${measure} as a numeric field.`} /> : <RequestFlow flows={flows} measure={measure} locale={locale} />}</TabsContent><footer className="flex min-h-12 flex-wrap items-center justify-between gap-3 border-t px-4 py-2 text-sm text-muted-foreground"><span>{periodLabel}</span><div className="flex gap-2"><Button variant="outline" size="sm" disabled>Previous</Button><Button variant="outline" size="sm" disabled>Next</Button></div></footer></section></Tabs><SupportingSummary modelCount={ranking.length} requestCount={metricComplete(currentRows, 'requests') ? currentTotals.requests : undefined} locale={locale} /></>
      )}
